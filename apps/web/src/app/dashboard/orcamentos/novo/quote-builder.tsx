  ...
  return (
    <form action={createQuoteV2Action} className="quote-builder">
      ...
      <section className="orbiq-panel">
        <div className="orbiq-panel-heading"><div><span className="orbiq-eyebrow">2 · SERVIÇOS</span><h2>Serviços realizados</h2><p className="quote-builder-section-description">Selecione a área e depois o serviço realizado pelo mecânico. A mão de obra é cadastrada separadamente.</p></div></div>
        ...
        {selectedServices.length > 0 ? <div className="quote-selected-services"><div className="quote-selected-title"><strong>Serviços selecionados</strong><span>{selectedServices.length}</span></div>{selectedServices.map((service) => <article key={service.key} className="quote-selected-service"><div className="quote-selected-service-heading"><div><span className="orbiq-eyebrow">{service.category}</span><h3>{service.description}</h3></div><div className="quote-selected-service-price"><strong>{money(service.laborAmount)}</strong><span>Serviço</span></div><button type="button" className="quote-remove-button" onClick={() => removeService(service.key)} aria-label="Remover serviço">×</button></div><div style={{ display: "flex", gap: 8, alignItems: "end", flexWrap: "wrap" }}><label style={{ flex: "1 1 180px" }}><span>Mão de obra</span><input value={service.laborAmount.toFixed(2).replace(".", ",")} onChange={(event) => { const parsed = parseMoney(event.target.value); if (parsed !== null) patchService(service.key, { laborAmount: parsed }); }} inputMode="decimal" /></label>{service.serviceCatalogId ? <button type="button" className="orbiq-secondary-button" onClick={() => saveLabor(service)} disabled={saving}>{saving ? "Salvando..." : "Salvar mão de obra"}</button> : null}</div><label className="quote-needs-part"><input type="checkbox" checked={service.needsPart} onChange={(event) => patchService(service.key, { needsPart: event.target.checked })} /><span><strong>Precisa comprar peça</strong><small>A peça entrará automaticamente na cotação.</small></span></label>{service.needsPart ? <div className="quote-service-part-grid"><label><span>Peça *</span><input value={service.partDescription} onChange={(event) => patchService(service.key, { partDescription: event.target.value })} required placeholder="Ex.: Coxim do motor" /></label><label><span>Categoria</span><select value={service.partCategory} onChange={(event) => patchService(service.key, { partCategory: event.target.value })}>{itemCategories.map((category) => <option key={category} value={category}>{category}</option>)}</select></label><label><span>Qtd.</span><input value={service.partQuantity} onChange={(event) => patchService(service.key, { partQuantity: event.target.value })} inputMode="decimal" /></label><label><span>Unidade</span><select value={service.partUnit} onChange={(event) => patchService(service.key, { partUnit: event.target.value })}>{units.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></label><label><span>Lado</span><select value={service.partSide} onChange={(event) => patchService(service.key, { partSide: event.target.value })}>{sides.map((side) => <option key={side || "none"} value={side}>{side || "Não se aplica"}</option>)}</select></label><label><span>Especificação</span><input value={service.partSpecification} onChange={(event) => patchService(service.key, { partSpecification: event.target.value })} placeholder="Marca, medida..." /></label></div> : null}</article>)}</div> : <div className="quote-builder-empty"><span>+</span><strong>Nenhum serviço selecionado</strong><small>Escolha uma área acima para começar.</small></div>}
      </section>
      ...
      <section className="quote-builder-finish"><div><span>Serviços</span><strong>{selectedServices.length}</strong></div><div><span>Itens para compra</span><strong>{itemsPayload.length}</strong></div><div><span>Mão de obra</span><strong>{money(laborTotal)}</strong></div><button type="submit" className="orbiq-primary-button quote-save-button" disabled={!canSave || saving}>Salvar orçamento</button></section>
    </form>
  );
}
