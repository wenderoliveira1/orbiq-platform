...
function failure(code: QuoteErrorCode): never { redirect(`/dashboard/orcamentos/novo?error=${code}`); }
...
export async function saveServiceCatalogAction(category: string, description: string, laborAmount: number): Promise<string> {
  ...
  if (!normalizedCategory || normalizedDescription.length < 2 || !Number.isFinite(laborAmount) || laborAmount < 0) {
...
export async function saveServiceLaborAction(serviceId: string, amount: number): Promise<void> {
  const { supabase, organization } = await getCurrentContext();
  if (!isUuid(serviceId) || !Number.isFinite(amount) || amount < 0 || amount > 999_999_999) throw new Error("Valor de mão de obra inválido.");
  const { error } = await supabase.from("service_catalog").update({ default_labor_amount: Math.round(amount * 100) / 100 }).eq("id", serviceId).eq("organization_id", organization.id);
...
export async function createQuoteV2Action(formData: FormData): Promise<never> {
  ...
  if (customerIdRaw.length > MAX_ID_CHARS || vehicleIdRaw.length > MAX_ID_CHARS || priorityInput.length > MAX_PRIORITY_CHARS || mileageRaw.length > MAX_MILEAGE_CHARS || notesRaw.length > MAX_NOTES_CHARS || servicesInput.length > MAX_SERVICES_JSON_CHARS || itemsInput.length > MAX_ITEMS_JSON_CHARS) return failure("payload_invalid");
  ...
  const payload = parseQuotePayload(servicesInput.trim(), itemsInput.trim(), priorityRaw);
