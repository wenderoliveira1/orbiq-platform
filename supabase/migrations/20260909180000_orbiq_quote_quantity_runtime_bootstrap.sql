...
alter table public.quote_services
  add constraint quote_services_quantity_check check (quantity > 0);
...
begin
  ...
  for item_entry in select value from jsonb_array_elements(items) loop
    ...
    insert into public.quote_items(
      organization_id,quote_id,category,description,quantity,unit,side,specification,notes,purchase_status
    ) values (
      ...
      nullif(upper(btrim(coalesce(item_entry->>'notes',''))),''),'pending'
    );
  end loop;
...
set final_amount = round(
  coalesce((select sum(coalesce(qs.labor_amount,0) * coalesce(qs.quantity,1)) from public.quote_services qs where qs.quote_id=q.id),0)
  + coalesce((select sum(coalesce(qi.chosen_amount,0) * coalesce(qi.quantity,1)) from public.quote_items qi where qi.quote_id=q.id and qi.chosen_amount is not null),0),
  2
);
