...
-- Recalculate a quote from source rows every time, without trusting quotes.final_amount.
...
begin
  ...
  select coalesce(sum(
    round(coalesce(qs.labor_amount, 0), 2) * coalesce(qs.quantity, 1)
  ), 0)
  into labor_total
  ...
  select coalesce(sum(
    round(coalesce(qi.chosen_amount, 0), 2) * coalesce(qi.quantity, 1)
  ), 0)
  into parts_total
...
-- Ensure inserts/updates/deletes always refresh the parent quote.
...
-- Quote creation: save unit price unchanged and quantity separately.
...
begin
  ...
  insert into public.quotes(
    organization_id, customer_id, vehicle_id, protocol, priority, status,
    mileage, notes, created_by
  ) values (
    target_org_id, target_customer_id, target_vehicle_id, new_protocol,
    target_priority, 'estimating', target_mileage,
    nullif(upper(btrim(coalesce(target_notes, ''))), ''), auth.uid()
  ...
  for service_entry in select value from jsonb_array_elements(services) loop
    ...
    insert into public.quote_services(
      organization_id, quote_id, labor_service_id, service_catalog_id,
      category, description, needs_part, quantity, labor_amount
    ) values (
      target_org_id, new_quote_id, null, service_catalog_id,
      service_category, service_description, service_needs_part,
